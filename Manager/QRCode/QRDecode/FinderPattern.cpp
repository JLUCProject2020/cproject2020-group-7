...
void FinderPattern::getCenter(Line * crossLines,int crossLinesCnt) 
{
	...
	{
#ifdef UNICODE
			AfxMessageBox(_T("Invalid number of Finder Pattern detected"));
#else
...
void FinderPattern::findLineAcross()
{
	...
	while(true) 
	{
		...
		if (direction == READ_HORIZONTAL) {
			...
			//	lengthBuffer =  new int[5];//gy 修改为下面的
				for(i=0;i<5;i++)
					lengthBuffer[i]=0;
			}
			else {
			...
			if (line.isNeighbor(lineNeighbor.back(), lineCandidate[j]))
			{
				...
				if (lineNeighbor.size() * 5 > compareLine.getLength() &&
					j == lineCandidate.size() - 1) {
						...
						{
							vector<Line>::iterator itorBegin = lineCandidate.begin();
							vector<Line>::iterator itorEnd = lineCandidate.end();
							vector<Line>::iterator lastItor;

							for (; itorBegin != itorEnd; itorBegin++)
							{
								if( (*itorBegin).x1==lineNeighbor[k].x1 && (*itorBegin).y1==lineNeighbor[k].y1
									&& (*itorBegin).x2==lineNeighbor[k].x2 && (*itorBegin).y2==lineNeighbor[k].y2)
								{
									lineCandidate.erase(itorBegin);
									itorBegin = lineCandidate.begin();
									itorEnd = lineCandidate.end();
								}
							}

							/*for(int index=0;index<lineCandidate.size() && itorBegin != itorEnd; index++, itorBegin++)
							{
								if( ((Line)lineCandidate[index]).x1==lineNeighbor[k].x1 && ((Line)lineCandidate[index]).y1==lineNeighbor[k].y1
									&& ((Line)lineCandidate[index]).x2==lineNeighbor[k].x2 && ((Line)lineCandidate[index]).y2==lineNeighbor[k].y2)
									lineCandidate.erase(itorBegin);
							}*/
