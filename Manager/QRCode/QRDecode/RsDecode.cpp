...
int RsDecode::chienSearch(int * pos, int n, int jisu, int * sigma)
{
	...
	for(int i = 0; i < n; i++) {
		...
		if(wk == 0) {
			int pv = m_galois.toExp(i);		// σ(z) = 0の解
...
void RsDecode::doForney(BYTE * data, int length, int jisu, int * pos, int * sigma, int * omega)
{
	...
	{
		int ps = pos[i];
		int zlog = 255 - m_galois.toLog(ps);					// zのスカラー

		// ω(z)の計算
		int ov = omega[0];
		for(j = 1; j < jisu; j++)
		{
			ov ^= m_galois.mulExp(omega[j], (zlog * j) % 255);		// ov += ωi * z^j
		}
		...
		for(j = 2; j < jisu; j += 2)
		{
			dv ^= m_galois.mulExp(sigma[j + 1], (zlog * j) % 255);	// dv += σ<j+1> * z^j
		}
