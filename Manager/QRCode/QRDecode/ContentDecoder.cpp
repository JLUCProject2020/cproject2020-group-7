	...
	{ //interleave data blocks because symbol has 2 or more RS blocks
		...
		{ //symbol has 2 types of RS blocks
			...
			if(shorterRSBlocks)
			{
				for(k=0;k<numShorterRSBlocks;k++)
				{
					if(shorterRSBlocks[k])
					{
						delete [] shorterRSBlocks[k];
						shorterRSBlocks[k]=NULL;
					}
				}
				delete shorterRSBlocks;
				shorterRSBlocks=NULL;
			}			
			if(longerRSBlocks)
			{
				for(k=0;k<numLongerRSBlocks;k++)
				{
					if(longerRSBlocks[k])
					{
						delete [] longerRSBlocks[k];
						longerRSBlocks[k]=NULL;
					}
				}
				delete longerRSBlocks;
				longerRSBlocks=NULL;
			}
		}
